...
class DeepIRT(nn.Module):
    ...
    """
    def __init__(self, key_dim, value_dim, summary_dim,
                 question_num, concept_num):
        super().__init__()
        ...
        # self._key_memory = nn.Parameter(torch.Tensor(self._key_dim,
        #                                self._concept_num)).to(ARGS.device)
        self._key_memory = nn.Linear(
            in_features=key_dim, out_features=concept_num, bias=False)
        stdev = 1 / (sqrt(self._concept_num + self._key_dim))
        nn.init.uniform_(self._key_memory.weight, -stdev, stdev)

        # IRT layers
        self._ability_layer = nn.Linear(
    ...
    def _init_value_memory(self):
        ...
        self._value_memory = nn.Parameter(
            torch.Tensor(self._value_dim, self._concept_num)).to(ARGS.device)
        stdev = 1 / (sqrt(self._concept_num + self._key_dim))
        nn.init.uniform_(self._value_memory, -stdev, stdev)
        self._value_memory = self._value_memory.clone().repeat(
            self._batch_size, 1, 1)  # (batch_size, key_dim, concept_num)
    ...
    def _compute_correlation_weight(self, question_id):
        ...
        return self._softmax(self._key_memory(question_vector)).to(ARGS.device)
    ...
    def _read(self, question_id):
        ...
        read_content = torch.matmul(
            self._value_memory, correlation_weight.unsqueeze(-1)).squeeze(-1)
    ...
    def forward(self, input, target_id):
        ...
        summary_vector = self._summary_layer(
            torch.cat((read_content, question_vector), dim=-1))
        summary_vector = self._tanh(summary_vector)

        ability_level = self._ability_layer(summary_vector)
        ability_level = self._tanh(ability_level)
        difficulty_level = self._difficulty_layer(question_vector)
        difficulty_level = self._tanh(difficulty_level)
        output = 3.0 * ability_level - difficulty_level
