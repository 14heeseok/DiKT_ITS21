...
class SAKT(nn.Module):
    ...
    """
    def __init__(self, hidden_dim, question_num, num_layers,
                 num_head, dropout):
        super().__init__()
        ...
        # Blocks
        self._layers = clones(
            SAKTLayer(hidden_dim, num_head, dropout), num_layers)
        ...
        self._positional_embedding = nn.Embedding(
            ARGS.seq_size+1, hidden_dim, padding_idx=PAD_INDEX)
        self._interaction_embedding = nn.Embedding(
            2*question_num+1, hidden_dim, padding_idx=PAD_INDEX)
    ...
    def _get_position_index(self, question_id):
        ...
        for i in range(batch_size):
            ...
            position_index = [0] * (ARGS.seq_size - non_padding_num) +\
