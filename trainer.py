...
class NoamOptimizer:
    def __init__(self, model, lr, model_size, warmup):
        ...
        self._opt = NoamOpt(
            model_size=model_size, factor=1, warmup=warmup,
...
class Trainer:
    def __init__(self, model, device, warm_up_step_count,
        ...
        self._opt = NoamOptimizer(model=model, lr=lr, model_size=d_model,
                                  warmup=warm_up_step_count)
    ...
    def train(self):
        ...
        self._total_steps = len(train_gen) * self._num_epochs
        print(self._total_steps)

        self.step = 0
        while self.step < self._total_steps:
            rem_steps = self._total_steps - self.step
            ...
            if self._early_stopping is True:
                break
    ...
    def _forward(self, batch):
        if ARGS.model == 'DiKT':
            batch['label'] = batch['label'].to(self._device)
            batch['target_id'] = batch['target_id'].to(self._device)
            batch['input'] = {k: t.to(self._device)
                              for k, t in batch['input'].items()}
        else:
        ...
        # shape: (batch_size, 1)
        pred = (torch.sigmoid(output) >= self._threshold).long()
    ...
    def _test(self, name, batches):
        ...
        if name == 'Validation':
            ...
            if self.step - self.max_step > self._total_steps*0.2:
